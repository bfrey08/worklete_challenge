...
describe 'LearnerQuizzes index page' do
  before(:each) do
    ...
    @quiz_questions1 = QuizQuestions.create(result: 'unanswered', question_id: @question1.id, quiz_id: @quiz1.id)
    @quiz_questions1 = QuizQuestions.create(result: 'unanswered', question_id: @question2.id, quiz_id: @quiz1.id)
    @quiz_questions1 = QuizQuestions.create(result: 'unanswered', question_id: @question3.id, quiz_id: @quiz1.id)
    @quiz_questions1 = QuizQuestions.create(result: 'unanswered', question_id: @question3.id, quiz_id: @quiz2.id)
    @quiz_questions1 = QuizQuestions.create(result: 'unanswered', question_id: @question4.id, quiz_id: @quiz2.id)
    @quiz_questions1 = QuizQuestions.create(result: 'unanswered', question_id: @question1.id, quiz_id: @quiz2.id)
  end

  it 'can show all available quizzes for a learner to take' do
    expect(page).to have_content("Available Quizzes")
    expect(page).to have_button("#{@quiz1.id}")
    expect(page).to have_button("#{@quiz2.id}")
  end
